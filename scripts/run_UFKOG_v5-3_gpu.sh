...
mkdir -p $CLUSTERS
mkdir -p $ASSEMBLY
mkdir -p $ASSEMBLY/hifiasm

# command for converting the read ids to the expected format
awk '/^>/{print ">S1_" ++i; next}{print}' < $READ_FILE > $INTERMEDIATES/reads.fasta

python3 /data/home/wangbo/software/rambler-main/scripts/from_read_unikmer_to_clusters_UFKOG_v5_3_gpu_v2.py -r $READ_FILE -u $KMER_FILE -o $OUT_DIR -k $k_len -s 1 -g 2
...
# do
	# hifiasm -o $ASSEMBLY/hifiasm/"$j" -t 48 $a
...
# python3 /data/home/wangbo/software/rambler-main/scripts/from_clusters_to_assembly.py $OUT_DIR $REF_SIZE
